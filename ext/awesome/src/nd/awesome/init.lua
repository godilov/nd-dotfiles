...
local colors_fn    = cache_res.get_colors
local keys_fn      = cache_res.get_keys
...
return function()
    if not is_init then
        ...
        local key_scheme = keys_fn(key_config['main'])
        local color_scheme = colors_fn(color_config['main'])
